...
""", unsafe_allow_html=True)
    ...
    st.markdown("---")
    st.markdown("### 💰 Filtro Premio")
    use_max_premium = st.toggle(
        "Filtra per premio massimo", value=False,
        help="Mostra solo spread con premio netto ≤ al valore impostato"
    )
    if use_max_premium:
        max_premium = st.number_input(
            "Premio massimo per azione ($)",
            min_value=0.05, max_value=50.0, value=2.0, step=0.05,
            help="Es: 2.00 → escludi spread che costano più di $2.00 per azione"
        )
        st.markdown(f"""
<div style="background:#1c1e21;border:1px solid #2e3035;border-radius:8px;padding:.6rem 1rem;margin-top:.3rem;font-size:.84rem;">
  <div style="color:#7a7e87;">Verranno mostrati solo spread con</div>
  <div style="color:#e0e2e5;font-weight:600;margin-top:.2rem;">
    premio netto &nbsp;&#8804;&nbsp;
    <span style="color:#4caf82;">${max_premium:.2f}</span> / azione
  </div>
</div>
""", unsafe_allow_html=True)
    else:
        ...
        with st.spinner(f"Analisi di {len(tickers_list)} titoli..."):
            ...
            for r in results:
                ...
                try:
                    ...
                    # Filtra per premio massimo se attivo
                    if max_premium is not None and s['net_debit_per_sh'] > max_premium:
                        continue
...
<div style="text-align:center;padding:4rem;color:#7a7e87;">
  ...
  <div style="margin-top:1rem;font-size:1rem;">Imposta i parametri e clicca <b>Scansiona Mercato</b></div>
        ...
        # Quanti esclusi dal filtro premio
        n_excluded = len(results) - len(strategies)
        if max_premium is not None and n_excluded > 0:
            st.info(f"🔎 {n_excluded} titol{'o' if n_excluded==1 else 'i'} esclus{'o' if n_excluded==1 else 'i'} "
                    f"perché il premio supera ${max_premium:.2f}/az. — "
                    f"mostro {len(strategies)} su {len(results)} candidati.")
        # Summary strip
