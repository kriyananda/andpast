...
def build_spread(ticker, price, hv, direction, R_dollar, tp_R, dte, r=0.05, width_pct=0.05):
    ...
    # Contratti basati su R (1 contratto costa net_debit → quanti ne compro per rischiare R_dollar?)
    contracts      = max(1, int(R_dollar / net_debit))
    ingresso_tot   = net_debit * contracts          # = max loss effettivo (= 1R effettivo)
    max_profit_tot = max_profit_1c * contracts
    actual_R       = round(ingresso_tot, 2)         # 1R effettivo dopo arrotondamento contratti

    # ── TP e SL corretti ──────────────────────────────────────────
    # tp_dollar = guadagno target in $ = 1R * multiplo_tp
    tp_dollar      = round(actual_R * tp_R, 2)
    # actual_tp_R = rapporto effettivo rispetto al max profit teorico
    actual_tp_R    = round(tp_dollar / actual_R, 2) if actual_R > 0 else 0

    # Prezzo del PREMIO dello spread all'uscita (per azione, per 1 contratto)
    # ingresso per azione = net_debit / 100 (net_debit è già per 100 azioni)
    net_debit_per_sh = net_debit / 100          # quanto hai pagato per azione
    # Guadagno target per azione = tp_dollar / (contracts * 100)
    tp_gain_per_sh   = tp_dollar / (contracts * 100)
    tp_exit_price    = round(net_debit_per_sh + tp_gain_per_sh, 2)   # prezzo spread al TP

    # SL: perdi tutto il premio → prezzo spread = 0.05 (floor minimo)
    sl_loss_per_sh   = actual_R / (contracts * 100)
    sl_exit_price    = max(0.05, round(net_debit_per_sh - sl_loss_per_sh, 2))
...
<div class="tp-box">
  ...
  <div style="margin-top:.6rem;">
    ...
    <div style="color:var(--pos);font-size:1.1rem;font-weight:700;margin-top:.15rem;">
      Chiudi spread a <b>${s['tp_exit_price']:.2f}</b> / azione
      <span class="r-badge">+${s['tp_dollar']:.2f}</span>
    </div>
    <div style="font-size:.78rem;color:var(--muted);margin-top:.15rem;">
      Entrato a ${s['net_debit_per_sh']:.2f} + guadagno ${s['tp_dollar'] / (s['contratti'] * 100):.2f} / az. = <b>${s['tp_exit_price']:.2f}</b>
    </div>
  </div>
  <div style="border-top:1px solid var(--bdr);margin:.6rem 0;"></div>
  <div>
    <div style="font-size:.78rem;color:var(--muted);text-transform:uppercase;letter-spacing:1px;">Stop Loss — 1R</div>
    <div style="color:var(--neg);font-size:1.1rem;font-weight:700;margin-top:.15rem;">
      Chiudi spread a <b>${s['sl_exit_price']:.2f}</b> / azione
      <span class="r-badge" style="background:rgba(192,95,95,.12);border-color:#c05f5f;color:#c05f5f;">−${s['R_dollar']:.2f}</span>
    </div>
    <div style="font-size:.78rem;color:var(--muted);margin-top:.15rem;">
      Entrato a ${s['net_debit_per_sh']:.2f} − perdita ${s['R_dollar'] / (s['contratti'] * 100):.2f} / az. = <b>${s['sl_exit_price']:.2f}</b>
    </div>
  </div>
</div>
""", unsafe_allow_html=True)
