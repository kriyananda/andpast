...
def build_spread(ticker, price, hv, direction, R_dollar, tp_R, dte, r=0.05, width_pct=0.05):
    ...
    ingresso_tot   = net_debit * contracts          # = max loss effettivo
    max_profit_tot = max_profit_1c * contracts
    tp_dollar      = ingresso_tot * tp_R            # target in $ = R * multiplo
    actual_R       = round(ingresso_tot, 2)         # 1R effettivo dopo arrotondamento contratti
    actual_tp_R    = round(max_profit_tot / actual_R, 2) if actual_R > 0 else 0
...
# TABS
# ════════════════════════════════════════════════════════════════
tab1, tab2, tab3 = st.tabs(["📊 Scanner & Strategie", "📈 Dashboard", "📋 Storico"])
  ...
  <div style="margin-top:1rem;font-size:1rem;">Imposta i parametri e clicca <b>Scansiona Mercato</b></div>
        ...
        for r in results:
            ...
            with st.expander(
                ...
                f"1R=${s['R_dollar']} | TP={s['tp_R']}R (${s['tp_dollar']:.0f}) | "
                f"R/R {s['rr_ratio']}x",
                expanded=False
            ):
...
<div class="kv-box" style="margin-bottom:.5rem;">
  ...
  <div style="margin-top:.4rem;font-size:.82rem;color:var(--muted);">
    Prezzo <b style="color:var(--txt)">${s['price']}</b> &nbsp;·&nbsp;
...
<table>
  ...
  <tr><td class="kv-label">Ingresso totale</td><td class="kv-val">${s['ingresso_totale']}</td></tr>
  <tr><td class="kv-label">1R (Max Loss)</td>  <td class="kv-neg">${s['R_dollar']}</td></tr>
  <tr><td class="kv-label">Max Profit</td>     <td class="kv-pos">${s['max_profit']}</td></tr>
  <tr><td class="kv-label">Breakeven</td>      <td class="kv-val">${s['breakeven']}</td></tr>
...
""", unsafe_allow_html=True)
                    ...
                    st.markdown(f"""
<div class="tp-box">
  <div class="tp-label">Target uscita — {s['tp_R']}R</div>
  <div class="tp-val">${s['tp_dollar']:.0f} <span class="r-badge">{s['tp_R']}R</span></div>
  <div class="tp-sub">Chiudi lo spread quando vale ${s['tp_dollar']:.0f}</div>
</div>
""", unsafe_allow_html=True)
